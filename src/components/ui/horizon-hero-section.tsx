...
export const HorizonHeroSection = () => {
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style relative min-h-screen overflow-hidden bg-black">
      <canvas ref={canvasRef} className="hero-canvas absolute inset-0 z-0" />
      ...
      <div ref={menuRef} className="side-menu fixed left-8 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-4" style={{ visibility: 'hidden' }}>
        <div className="menu-icon flex flex-col gap-1.5 cursor-pointer group">
          <span className="w-6 h-px bg-white/50 group-hover:bg-white transition-colors"></span>
          <span className="w-6 h-px bg-white/50 group-hover:bg-white transition-colors"></span>
          <span className="w-6 h-px bg-white/50 group-hover:bg-white transition-colors"></span>
        </div>
        <div className="vertical-text [writing-mode:vertical-lr] text-white/30 text-[10px] tracking-[0.5em] font-light">SPACE</div>
      </div>

      {/* Main content */}
      <div className="hero-content cosmos-content relative z-10 flex flex-col items-center justify-center min-h-screen text-center px-4">
        <h1 ref={titleRef} className="hero-title text-7xl md:text-9xl font-bold tracking-[0.2em] text-white mb-8">
          {"HORIZON".split('').map((char, i) => (
            <span key={i} className="title-char inline-block">{char}</span>
          ))}
        </h1>
        
        <div ref={subtitleRef} className="hero-subtitle cosmos-subtitle text-white/60 text-sm md:text-base tracking-[0.3em] font-light">
      ...
      <div ref={scrollProgressRef} className="scroll-progress fixed bottom-12 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-4" style={{ visibility: 'hidden' }}>
        <div className="scroll-text text-white/30 text-[10px] tracking-[0.5em] font-light">SCROLL</div>
        <div className="progress-track w-px h-24 bg-white/10 relative">
          <div 
            className="progress-fill absolute top-0 left-0 w-full bg-white transition-all duration-300" 
            style={{ height: `${scrollProgress * 100}%` }}
          />
        </div>
        <div className="section-counter text-white/50 font-mono text-[10px]">
      ...
      <div className="scroll-sections relative z-10">
       {[...Array(2)].map((_, i) => {
          ...
          return (
            <section key={i} className="content-section min-h-screen flex flex-col items-center justify-center text-center px-4">
              <h1 className="hero-title text-7xl md:text-9xl font-bold tracking-[0.2em] text-white mb-8">
                {titles[i+1] || 'DEFAULT'}
              </h1>
          
              <div className="hero-subtitle cosmos-subtitle text-white/60 text-sm md:text-base tracking-[0.3em] font-light">
                <p className="subtitle-line mb-2">
                  {subtitles[i+1].line1}
                </p>
                <p className="subtitle-line">
